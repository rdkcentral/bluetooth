...
DoSBCwrite (
    ...
    if (mkfifo(myfifo, 0666) != 0) {
        fprintf(stderr, "%d\t: %s - FIFO creation failed ..\n",  __LINE__, __FUNCTION__);
    }
    fprintf(stderr, "%d\t: %s - Thread starting: %s \n", __LINE__, __FUNCTION__, message);
    
    fd = open(myfifo, O_WRONLY );//not sure if I need nonblock or not the first time I tried it it failed. then I tried nonblock, no audioa
    if (fd == -1) {
        fprintf(stderr, "%d\t: %s - file opening is failed...\n", __LINE__, __FUNCTION__);
        return 0;
    }
            ...
            while (first_time_thru) {
                ...
                if (x == 0x9C) {
                    ...
                    switch (bit_channel_mode) {
                        case 0x00:
                        // CID 23429: Unused value (UNUSED_VALUE)
                        tmp = blocks * bitpool;
                        break;
                        case 0x01:
                        ...
                        break;
                        // CID 340936: Dead default in switch (DEADCODE)
                    }
...
getLeUuidString (
    ...
    /* On DELIA-60950, Replacing scanf with fgets looking for '\n' at the end
     * which causes strcmp failed on btrCore. Hence replacing the '\n' of the string as
     * '\0' at the end.
     */
    size_t len = strlen(leUuidString);
    if (len > 0 && leUuidString[len-1] == '\n') {
        leUuidString[--len] = '\0';
    }
...
sendSBCFileOverBT (
    ...
    while (bytesLeft) {
        ...
        // write bluetooth
        if (timeout > 0) {
            size_t bytes_read = fread(encoded_buf, 1, bytesToSend, sbcFilePtr); // CID 23344 : Ignoring number of bytes read (CHECKED_RETURN)
            if (bytes_read <= 0) {
                fprintf(stderr, "Error: End of file reached before reading all data.\n");
                fclose(sbcFilePtr);
                free(encoded_buf);
                return;
            }
            if (write(fd, encoded_buf, bytes_read)) {
            }
            bytesLeft -= bytes_read;
            sbcFilePtr = sbcFilePtr + bytes_read;
        }
...
printMenu (
    ...
    fprintf( stderr, "50. Install agent for capability choice (GDbus way): 0 = DisplayOnly, 1 = DisplayYesNo, 2 = KeyboardOnly, 3 = NoInputNoOutput and 4 = KeyboardDisplay \n");
    fprintf( stderr, "52. Uninstall agent - allows device-initiated pairing\n");
    fprintf( stderr, "53. Flashing firmware upgrade\n");
...
#endif
    int capNum = 3; // by default, "NoInputNoOutput"
    char lpcBtVersion[64] = {'\0'};
    ...
    do {
        ...
        case 3:
            {
                ...
                fprintf(stderr, "%d\t: %s - numberOfScannedDevices : %d \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.numberOfDevices);
                fprintf(stderr, "%d\t: %s - Display Ad Service data for scanned devices\n", __LINE__, __FUNCTION__);
                fprintf(stderr, "%d\t: %s - *******************************************\n", __LINE__, __FUNCTION__);

                for(int i = 0; i < lstBTRCoreScannedDevList.numberOfDevices; i++)
                {
                    fprintf(stderr, "%d\t: %s - Scanned Device : Index %d \n", __LINE__, __FUNCTION__, i);
                    fprintf(stderr, "%d\t: %s - Scanned Device : Rssi  %d \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.devices[i].i32RSSI);
                    fprintf(stderr, "%d\t: %s - Scanned Device : Name  %s \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.devices[i].pcDeviceName);
                    fprintf(stderr, "%d\t: %s - Scanned Device : Addr  %s \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.devices[i].pcDeviceAddress);
                    ...
                    {
                        fprintf(stderr, "%d\t: %s - UUID value : 0x%x \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.devices[i].stDeviceProfile.profile[count].uuid_value);
                        fprintf(stderr, "%d\t: %s - ServiceData for UUID : %s \n", __LINE__, __FUNCTION__, lstBTRCoreScannedDevList.devices[i].stDeviceProfile.profile[count].profile_name);
        ...
        case 6:
            {
                ...
                fprintf(stderr, "%d\t: %s - numberOfPairedDevices : %d \n", __LINE__, __FUNCTION__, lstBTRCorePairedDevList.numberOfDevices);
                fprintf(stderr, "%d\t: %s - *******************************************\n", __LINE__, __FUNCTION__);

                for(int i = 0; i < lstBTRCorePairedDevList.numberOfDevices; i++)
                {
                    fprintf(stderr, "%d\t: %s - Paired Device : Index %d \n", __LINE__, __FUNCTION__, i);
                    fprintf(stderr, "%d\t: %s - Paired Device : Rssi  %d \n", __LINE__, __FUNCTION__, lstBTRCorePairedDevList.devices[i].i32RSSI);
                    fprintf(stderr, "%d\t: %s - Paired Device : Name  %s \n", __LINE__, __FUNCTION__, lstBTRCorePairedDevList.devices[i].pcDeviceName);
                    fprintf(stderr, "%d\t: %s - Paired Device : Addr  %s \n", __LINE__, __FUNCTION__, lstBTRCorePairedDevList.devices[i].pcDeviceAddress);
                    fprintf(stderr, "%d\t: %s - ---------------------------------------\n", __LINE__, __FUNCTION__);
                }
        ...
        case 23:
            ...
            if (sbcEncodedFileName) {
                ...
                // CID 41344: Unused value (UNUSED_VALUE)
            }
            else {
        ...
        case 42:
           {
                ...
                if (aenBTRCoreLeOp == enBTRCoreLeOpGWriteValue) {
                   ...
                   /* if (scanf("%s", writeArg)) {
                    ...
                    if(fgets(writeArg, sizeof(writeArg), stdin)) {
                    }
                }

                if (leUuidString) {
                   char val[BTRCORE_MAX_STR_LEN] = "\0";
                   if (enBTRCoreSuccess != BTRCore_PerformLEOp (lhBTRCore, devnum, leUuidString, aenBTRCoreLeOp, writeArg, val)) {
                       fprintf(stderr, "%d\t: %s - Perform LE Op for device  %d Failed!!!\n", __LINE__, __FUNCTION__, devnum);
                   }
                   free(leUuidString);
                   leUuidString = NULL;
                   if (aenBTRCoreLeOp == 1) {
        ...
        case 50:
            fprintf(stderr, "%d\t: %s - install agent for capability choice: 0 = DisplayOnly, 1 = DisplayYesNo, 2 = KeyboardOnly, 3 = NoInputNoOutput and 4 = KeyboardDisplay\n", __LINE__, __FUNCTION__);
            capNum = getChoice();
            BTRCore_RegisterAgent(lhBTRCore, capNum); // 2nd arg controls the mode
            BTRCore_GetVersionInfo(lhBTRCore, lpcBtVersion);
            break;
        case 52:
            fprintf(stderr, "%d\t: %s - uninstall agent\n", __LINE__, __FUNCTION__);
            BTRCore_UnregisterAgent(lhBTRCore);
            break;        
        case 99:
