...
typedef struct _stBTRCoreLeCustomAdv {
    char                    BeaconName[BT_MAX_STR_LEN];                                 /* Name of advertising packet : "XLESETUP-8713", etc. */
...
btrCore_LE_isServiceSupported (
    ...
    if (!strcmp(lUUID, BTR_CORE_GATT_TILE_1) ||
        ...
        !strcmp(lUUID, BTR_CORE_DEVICE_INFO) ||
        !strcmp(lUUID, BTR_CORE_BATTERY_SERVICE) ||
        !strcmp(lUUID, BTR_CORE_BATTERY_SERVICE_XBB_1) ||
        !strcmp(lUUID, BTR_CORE_BATTERY_SERVICE_XBB_2) ||
        !strcmp(lUUID, BTR_CORE_BATTERY_SERVICE_XBB_3)){
        isSupported = TRUE;
...
btrCore_LE_GetAllowedGattFlagValues (
    ...
    for (u8idx = 0; u8idx < maxFlags && (flags[u8idx][0] != '\0'); u8idx++) {
    ...
    if (TRUE == lbFoundGattPath) {
        ...
        case enBTLeGattOpWriteValue: {
            ...
            case enBTGattCharacteristic: {
                ...
                break;
            case enBTGattDescriptor: {
                if(lpstBTRCoreLeGattDesc) {  //CID 115335: Explicit null dereferenced
                   strncpy(lpstBTRCoreLeGattDesc->propertyValue, apValue, BT_MAX_GATT_OP_DATA_LEN - 1);
                   lstBtrLeInfo.pui8Uuid = lpstBTRCoreLeGattDesc->descUuid;
...
BTRCore_LE_StartAdvertisement (
    ...
    // parsing the adv data from uplayer
    stBTLeCustomAdv lstBTRCoreLeAdvInfo;
    stBTRCoreLeCustomAdv *plstBTRCoreLeAdvInfo = &lpstlhBTRCoreLe->stCustomAdv;
    uint8_t index;

    memset(&lstBTRCoreLeAdvInfo, 0, sizeof(stBTLeCustomAdv));

    lstBTRCoreLeAdvInfo.bTxPower = plstBTRCoreLeAdvInfo->bTxPower;
    lstBTRCoreLeAdvInfo.numServiceUUID = plstBTRCoreLeAdvInfo->numServiceUUID;
    lstBTRCoreLeAdvInfo.numSolicitUUID = plstBTRCoreLeAdvInfo->numSolicitUUID;
    memcpy(&lstBTRCoreLeAdvInfo.ManfData, &plstBTRCoreLeAdvInfo->ManfData, sizeof(stBTRCoreLeManfData));
    memcpy(&lstBTRCoreLeAdvInfo.ServiceData, &plstBTRCoreLeAdvInfo->ServiceData, sizeof(stBTRCoreLeServData));
    strncpy(lstBTRCoreLeAdvInfo.AdvertisementType, plstBTRCoreLeAdvInfo->AdvertisementType, BT_MAX_STR_LEN - 1);

    for (index = 0; index < lstBTRCoreLeAdvInfo.numServiceUUID; index++)
        strncpy(lstBTRCoreLeAdvInfo.ServiceUUID[index], plstBTRCoreLeAdvInfo->ServiceUUID[index], BT_MAX_STR_LEN - 1);

    for (index = 0; index < lstBTRCoreLeAdvInfo.numSolicitUUID; index++)
        strncpy(lstBTRCoreLeAdvInfo.SolicitUUID[index], plstBTRCoreLeAdvInfo->SolicitUUID[index], BT_MAX_STR_LEN - 1);

     strncpy(lstBTRCoreLeAdvInfo.BeaconName, plstBTRCoreLeAdvInfo->BeaconName, BT_MAX_STR_LEN - 1);

    // parsing the gattinfo from uplayer
    stBTLeGattInfo lstBTRCoreLeGattInfo;
    stBTLeGattService *apstBTRCoreLeGattService = lstBTRCoreLeGattInfo.astBTRGattService;
    stBTRCoreLeGattService *lpstBTRLeGattService = lpstlhBTRCoreLe->stBTRLeGattService;
    ...
    lstBTRCoreLeGattInfo.nNumGattServices = lpstlhBTRCoreLe->ui16NumOfLocalGattServices;

    /* Invoke method calls for RegisterAdvertisement and RegisterApplication from here for now */
    if (BtrCore_BTRegisterLeAdvertisement(apBtConn, apBtAdapter, &lstBTRCoreLeAdvInfo)) {
        BTRCORELOG_ERROR("Unable to register advertisement\n");
        lenBTRCoreRet = enBTRCoreFailure;
    }

    if (BtrCore_BTRegisterLeGatt(apBtConn, apBtAdapter, &lstBTRCoreLeGattInfo)) {
...
BTRCore_LE_StopAdvertisement (
    ...
    // 'clean' gatt services
    pstlhBTRCoreLe->stCustomAdv.numServiceUUID = 0;

    return lenBTRCoreRet;
}

enBTRCoreRet
BTRCore_LE_ReleaseAdvertisement (
    tBTRCoreLeHdl  hBTRCoreLe,
    void*          apBtConn,
    const char*    apBtAdapter
) {
    ...
    if (BtrCore_BTReleaseAdvertisement (apBtConn,
                                    apBtAdapter)) {
        lenBTRCoreRet = enBTRCoreFailure;
    }
...
enBTRCoreRet
BTRCore_LE_SetAdvertisementInfo (
    tBTRCoreLeHdl   hBTRCoreLe,
    char*           aAdvtType,
    ...
    if (NULL != lpstlhBTRCoreLe) {
        ...
        if (aAdvtType &&
            (!(strncmp(aAdvtType, "peripheral", BTRCORE_MAX_STR_LEN)) ||
             !(strncmp(aAdvtType, "broadcast", BTRCORE_MAX_STR_LEN)))) {
            strncpy(lpstBTRCoreLeCustAdv->AdvertisementType, aAdvtType, BTRCORE_MAX_STR_LEN - 1);
            BTRCORELOG_INFO("Adv type : %s\n", lpstBTRCoreLeCustAdv->AdvertisementType);
            lenBTRCoreRet = enBTRCoreSuccess;
        }

        if(NULL != aAdvtBeaconName) {
            strncpy(lpstBTRCoreLeCustAdv->BeaconName, aAdvtBeaconName, BTRCORE_MAX_STR_LEN - 1);
            BTRCORELOG_INFO("Adv Name : %s\n", lpstBTRCoreLeCustAdv->BeaconName);
            lenBTRCoreRet = enBTRCoreSuccess;
        }
    }
...
BTRCore_LE_SetServiceUUIDs (
    ...
    if (NULL != lpstlhBTRCoreLe) {
        ...
        if (BTR_MAX_GATT_SERVICE > lpstBTRCoreLeCustAdv->numServiceUUID) {
            strncpy(lpstBTRCoreLeCustAdv->ServiceUUID[lpstBTRCoreLeCustAdv->numServiceUUID], aUUID, BTRCORE_MAX_STR_LEN - 1);
...
BTRCore_LE_AddGattServiceInfo (
    ...
    if(enBTRCoreSuccess == btrCore_LE_GetGattInfo(lpstlhBTRCoreLe, aUUID, enBTRCoreLEGPropService, (void*)&pService)) {
       return (int*)pService;
    }
    ...
    if (NULL == pService) {
        if (BTR_MAX_GATT_SERVICE > lpstlhBTRCoreLe->ui16NumOfLocalGattServices) {
            ...
            strncpy(lCurAdapterAddress, aBtrDevAddr, BT_MAX_DEV_PATH_LEN - 1);
            ...
            MEMSET_S(lpBtLeGattSrvEpPath, BT_MAX_DEV_PATH_LEN, '\0', BT_MAX_DEV_PATH_LEN);
            strncpy(lpBtLeGattSrvEpPath, apBtAdapter, BT_MAX_DEV_PATH_LEN - 1);
            ...
            strncpy(lpstBTRGattService->serviceUuid, aUUID, BT_MAX_UUID_STR_LEN -1);
...
int*
BTRCore_LE_RemoveGattServiceInfo (
    tBTRCoreLeHdl   hBTRCoreLe,
    const char*     apBtAdapter,
    char*           aUUID,
    int*            aNumGattServices
    ...
    if (NULL != pService)
    {
        lSrvCnt = lpstlhBTRCoreLe->ui16NumOfLocalGattServices;
        for (int lIndex = 0; lIndex < lSrvCnt; lIndex++)
        {
            lpstBTRGattService = &lpstlhBTRCoreLe->stBTRLeGattService[lIndex];
            if (strncmp(lpstBTRGattService->serviceUuid, aUUID, strlen(aUUID)) == 0)
            {
                MEMSET_S(lpstBTRGattService, sizeof(stBTRCoreLeGattService), '\0', sizeof(stBTRCoreLeGattService));
                MEMCPY_S(lpstBTRGattService, sizeof(stBTRCoreLeGattService), &lpstlhBTRCoreLe->stBTRLeGattService[lSrvCnt-1], sizeof(stBTRCoreLeGattService));
                MEMSET_S(&lpstlhBTRCoreLe->stBTRLeGattService[lSrvCnt-1], sizeof(stBTRCoreLeGattService), '\0', sizeof(stBTRCoreLeGattService));                
                lpstlhBTRCoreLe->ui16NumOfLocalGattServices -= 1;
                *aNumGattServices = lpstlhBTRCoreLe->ui16NumOfLocalGattServices;
                break;
            }
...
BTRCore_LE_AddGattCharInfo (
    ...
    if(enBTRCoreSuccess == btrCore_LE_GetGattInfo(lpstlhBTRCoreLe, aUUID, enBTRCoreLEGPropChar, (void*)&pChar)) {
       return (int*)pChar;
    }
    if (NULL == pChar) {
        ...
        if (NULL != pParent) {
            ...
            if (BTR_MAX_GATT_CHAR > lpstBTRGattService->ui16NumberOfGattChar) {
                ...
                /* Set gatt char UUID */
                strncpy(lpstBTRCoreLeGattChar->charUuid, aUUID, BT_MAX_UUID_STR_LEN - 1);
                /* Set parent service */
                ...
                if ((NULL != aValue) && (BTR_GATT_CHAR_FLAG_READ == (BTR_GATT_CHAR_FLAG_READ & aCharFlags))) {
                    strncpy(lpstBTRCoreLeGattChar->value, aValue, BT_MAX_GATT_OP_DATA_LEN - 1);
                }
...
BTRCore_LE_AddGattDescInfo (
    ...
    /* Check whether the char uuid exists */
    if(enBTRCoreSuccess == btrCore_LE_GetGattInfo(lpstlhBTRCoreLe, aUUID, enBTRCoreLEGPropDesc, (void*)&pDesc)) {
       return (int*)pDesc;
    }
    if (NULL == pDesc) {
        /* Check whether parent service uuid exists */
        ...
        if (NULL != pParent) {
            ...
            if (BTR_MAX_GATT_DESC > lpstBTRGattChar->ui16NumberOfGattDesc) {
                ...
                /* Set desc UUID */
                strncpy(lpstBTRGattDesc->descUuid, aUUID, BT_MAX_UUID_STR_LEN - 1);
                /* Set desc parent char */
                lpstBTRGattDesc->parentChar = pParent;
                /* Set desc path */
                strncpy(lpstBTRGattDesc->descPath, lpstBTRGattChar->charPath, sizeof(lpstBTRGattDesc->descPath)-1);
                int ret = snprintf((lpstBTRGattDesc->descPath + strlen(lpstBTRGattChar->charPath)), BT_MAX_STR_LEN - strlen(lpstBTRGattChar->charPath), "/%s%03d", "desc", lIndex);
                if (ret > (BT_MAX_STR_LEN - 1)) {
                ...
                if ((NULL != aValue) && (BTR_GATT_CHAR_FLAG_READ == (BTR_GATT_CHAR_FLAG_READ & aDescFlags))) {
                    strncpy(lpstBTRGattDesc->propertyValue, aValue, BT_MAX_GATT_OP_DATA_LEN - 1);
                }
...
BTRCore_LE_SetPropertyValue (
    ...
    stBTRCoreLeHdl*         lpstlhBTRCoreLe = (stBTRCoreLeHdl*)hBTRCoreLe;
    stBTRCoreLeGattChar*    lpstBTRGattChar = NULL;
    unBTOpIfceProp          lunBtOpGattCharProp;
    ...
    lunBtOpGattCharProp.enBtGattCharProp = enBTGattCPropValue;
    ...
    if (NULL != lpstBTRGattChar) {
        if (BtrCore_BTSetProp(lpstlhBTRCoreLe->btIfceHdl, lpstBTRGattChar->charPath, enBTGattCharacteristic, lunBtOpGattCharProp, (void*)aValue)) {
...
BtrCore_LE_BatteryOTATransfer (
    ...
    BTRCORELOG_INFO(" In BtrCore_LE_BatteryOTATransfer \n");
    ...
    if (BtrCore_BTBatteryOTADataTransfer(lpstlhBTRCoreLe->btIfceHdl,
                                         lpcBtLePath,
                                         apcBtUuid,
                                         FileName)) {
        BTRCORELOG_ERROR("Failed to perform battery OTA Dtat Transfer on %s \n",apcBtUuid);
        return enBTRCoreFailure;
    }
    return enBTRCoreSuccess;
}
...
BtrCore_LE_BatterySetLED (
    ...
    BTRCORELOG_INFO("In BtrCore_LE_BatterySetLED \n");
    ...
    if (BtrCore_BTBatterySetLED(lpstlhBTRCoreLe->btIfceHdl,
                                         lpcBtLePath,
                                         apcBtUuid)) {
        BTRCORELOG_ERROR("Failed to perform battery Set LED on %s \n",apcBtUuid);
        return enBTRCoreFailure;
    }
    return enBTRCoreSuccess;
}
...
enBTRCoreRet
BtrCore_LE_BatteryWriteOTAControl (
    tBTRCoreLeHdl      hBTRCoreLe,
    tBTRCoreDevId      atBTRCoreDevId,
    const char*        apcBtUuid,
    int                value
    ...
    BTRCORELOG_INFO("In BtrCore_LE_BatteryWriteOTAControl \n");
    ...
    if (BtrCore_BTBatteryWriteOTAControl(lpstlhBTRCoreLe->btIfceHdl,
                                         lpcBtLePath,
                                         apcBtUuid,
                                         value)) {
        BTRCORELOG_ERROR("Failed to perform battery write OTA control on  %s \n",apcBtUuid);
        return enBTRCoreFailure;
    }
    return enBTRCoreSuccess;
}
...
btrCore_LE_GattInfoCb (
    ...
    if (enBTAdvertisement == aenBtOpIfceType) {
        ...
        char lBtUuid[BT_MAX_UUID_STR_LEN] = "\0";
            ...
            if (!retUuid && btrCore_LE_isServiceSupported(lBtUuid)) {
                ...
                if (!retDPath) {
                    ...
                    if (pProfile && pProfile->ui16NumberOfGattService < BTR_MAX_GATT_SERVICE) {
                        pService = btrCore_LE_FindGattService(pProfile, apBtGattPath);
                        if(!pService) {
                ...
                if ((pProfile = btrCore_LE_FindGattProfile(lpstlhBTRCoreLe, ltBTRCoreDevId))) {
                    if ((pService = btrCore_LE_FindGattService(pProfile, lBtSerivcePath))) {
                        ...
                        if ((pService->ui16NumberOfGattChar < BTR_MAX_GATT_CHAR)) {
                            ...
                            pChar = btrCore_LE_FindGattCharacteristic(pService, apBtGattPath);
                            if(!pChar) {
                                char cFlags[BTR_MAX_GATT_CHAR_FLAGS][BT_MAX_UUID_STR_LEN];
                                MEMSET_S(cFlags, sizeof(cFlags), 0, sizeof(cFlags));
                                ...
                                if (!BtrCore_BTGetProp(lpstlhBTRCoreLe->btIfceHdl, apBtGattPath, aenBtOpIfceType, aunBTOpIfceProp, (void*)&cFlags)) {
                                    ...
                                    if (pChar->charFlags & BTR_GATT_CHAR_FLAG_NOTIFY) {
                                        stBTRCoreLeGattInfo lstBtrLeInfo;
                                        /*TODO : When smartphone act as a GATT server and if the notification is turned off(notify flag is disabled),
                                         * we should be handling the scenario gracefully.
                                         */
                                        BTRCORELOG_INFO(" Notify characteristics added UUID - %s aBtdevAddr %s \n",pChar->charUuid,aBtdevAddr);
                                        lstBtrLeInfo.enLeOper = enBTRCoreLEGOpStartNotify;
                                ...
                                BTRCORELOG_INFO ("Added Characteristic %s Successfully.\n", lBtUuid);
                            }
                            else {
                    ...
                    if ((pService = btrCore_LE_FindGattService(pProfile, lBtSerivcePath))) {
                        if ((pChar = btrCore_LE_FindGattCharacteristic(pService, lBtCharPath))) {
                            ...
                            if (pChar->ui16NumberOfGattDesc < BTR_MAX_GATT_DESC) {
                                ...
                                pDesc = btrCore_LE_FindGattDescriptor(pChar, apBtGattPath);
                                if(!pDesc) {
                                    char dFlags[BTR_MAX_GATT_DESC_FLAGS][BT_MAX_UUID_STR_LEN];
                                    MEMSET_S(dFlags, sizeof(dFlags), 0, sizeof(dFlags));
        ...
        if (aenBtOpIfceType == enBTGattService) {
            ...
            }else if (enBTLeGattOpStartNotify == aenGattOp) {
                /* -------------Callback to Higher Layers-------------- */
                stBTRCoreLeGattInfo lstBtrLeInfo;
                lstBtrLeInfo.enLeOper = enBTRCoreLEGOpStartNotify;
                lstBtrLeInfo.pui8Uuid = (char*)apLeCbData;
                lpstlhBTRCoreLe->fpcBTRCoreLeStatusUpdate(&lstBtrLeInfo, aBtdevAddr, lpstlhBTRCoreLe->pvBtLeStatusUserData);
            }
            else if (enBTLeGattOpStopNotify == aenGattOp) {
                /* -------------Callback to Higher Layers-------------- */
                stBTRCoreLeGattInfo lstBtrLeInfo;
                lstBtrLeInfo.enLeOper = enBTRCoreLEGOpStopNotify;
                lstBtrLeInfo.pui8Uuid = (char*)apLeCbData;
                lpstlhBTRCoreLe->fpcBTRCoreLeStatusUpdate(&lstBtrLeInfo, aBtdevAddr, lpstlhBTRCoreLe->pvBtLeStatusUserData);
            }
                        ...
                        if ((pService = btrCore_LE_FindGattService(pProfile, lBtSerivcePath))) {
                            if ((pChar = btrCore_LE_FindGattCharacteristic(pService, apBtGattPath))) {
                                ...
                                if ((pChar->charFlags & BTR_GATT_CHAR_FLAG_READ) ||
                                    ...
                                    MEMSET_S(value, BT_MAX_GATT_OP_DATA_LEN, '\0', BT_MAX_GATT_OP_DATA_LEN);
                                    MEMSET_S(&lstBtrLeInfo, sizeof(stBTRCoreLeGattInfo), 0, sizeof(stBTRCoreLeGattInfo));
