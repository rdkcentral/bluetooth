...
#define BT_LE_GATT_SERVER_ENDPOINT          "/org/bluez/hci0/dev_FA_EB_DC_CD_BE_AF"     //High nibble descend F-A, Lower nibble ascend A-F
...
#define BT_BLUEZ_VERSION_5_55               "5.55"
                                ...
                                while (dbus_message_iter_has_next(&innerArrayIter2)) {
                                    if (DBUS_TYPE_DICT_ENTRY == dbus_message_iter_get_arg_type(&innerArrayIter2)) {
                                        ...
                                        if (DBUS_TYPE_STRING == dbus_message_iter_get_arg_type(&innerDictEntryIter3)) {
                                            ...
                                            MEMSET_S(objectData, BT_MAX_DEV_PATH_LEN, '\0', BT_MAX_DEV_PATH_LEN);
...
btrCore_BTAgentAuthorize (
    ...
    if (lpcPath) {
        ...
        if (lenBTDevType == enBTDevUnknown && !strncmp(lstBTDeviceInfo.pcIcon,"input-gaming",strlen("input-gaming"))) {
            lenBTDevType = enBTDevHID;
        }
    ...
    if (!lpDBusReply) {
        ...
        if (enBTDevAudioSource == lenBTDevType && yesNo) {
            STRCPY_S(lstBTDeviceInfo.pcDeviceCurrState,sizeof(lstBTDeviceInfo.pcDeviceCurrState),"connected");
    ...
    for (dbus_message_iter_recurse(apDBusMsgIter, &dict_iter); // recurse into array (of dictionary entries)
        ...
        if (dbus_type == DBUS_TYPE_DICT_ENTRY) {
            ...
            if (strcmp (pcKey, "Address") == 0 && dbus_type == DBUS_TYPE_STRING) {
                ...
                BTRCORELOG_INFO ("bDiscovering            = %d\n", apstBTAdapterInfo->bDiscovering);
            }
...
btrCore_BTParseDevice (
    ...
    while (dbus_message_iter_get_arg_type(&element_i) != DBUS_TYPE_INVALID) {
        if (dbus_message_iter_get_arg_type(&element_i) == DBUS_TYPE_DICT_ENTRY) {
            ...
            else if (strcmp (pcKey, "Modalias") == 0) {
                dbus_message_iter_next(&dict_i);
                dbus_message_iter_recurse(&dict_i, &variant_i);
                dbus_message_iter_get_basic(&variant_i, &pcModalias);
                strncpy(apstBTDeviceInfo->pcModalias, pcModalias, BT_MAX_STR_LEN-1);
                BTRCORELOG_TRACE ("Modalias       = %s bluez : %s\n", apstBTDeviceInfo->pcModalias, pcModalias);
            }
            else if (strcmp (pcKey, "Appearance") == 0) {
                dbus_message_iter_next(&dict_i);
                dbus_message_iter_recurse(&dict_i, &variant_i);
                dbus_message_iter_get_basic(&variant_i, &ui16Appearance);
                apstBTDeviceInfo->ui16Appearance = ui16Appearance;
                BTRCORELOG_TRACE ("ui16Appearance = %d\n", apstBTDeviceInfo->ui16Appearance);
            }
                            ...
                            if (dbus_type == DBUS_TYPE_ARRAY) {
                                ...
                                if(service_data && len) {
                                    if(len <= BT_MAX_SERVICE_DATA_LEN) {
                                        ...
                                        MEMCPY_S(apstBTDeviceInfo->saServices[count].pcData,sizeof(apstBTDeviceInfo->saServices[count].pcData), service_data, len);
                                    }
                                    else {
    ...
    while (dbus_message_iter_get_arg_type(&element_i) != DBUS_TYPE_INVALID) {
        if (dbus_message_iter_get_arg_type(&element_i) == DBUS_TYPE_DICT_ENTRY) {
            ...
            if (strcmp (pcKey, "Codec") == 0) {
                ...
                BTRCORELOG_TRACE ("apstBTMediaInfo->ui8Codec : %d\n", apstBTMediaInfo->ui8Codec);
            }
                ...
                BTRCORELOG_TRACE ("apstBTMediaInfo->pcState: %s\n", apstBTMediaInfo->pcState);
                ...
                BTRCORELOG_TRACE ("apstBTMediaInfo->pcUUID: %s\n", apstBTMediaInfo->pcUUID);
            }
                ...
                BTRCORELOG_TRACE ("apstBTMediaInfo->ui16Delay = %d\n", apstBTMediaInfo->ui16Delay);
            }
                ...
                BTRCORELOG_TRACE ("apstBTMediaInfo->ui16Volume = %d\n", apstBTMediaInfo->ui16Volume);
            }
        }
                ...
                if (0 == strcmp(dbusObject, "device")) {
                    if (true == dbus_message_iter_next(&dictEntryIter)) {
                        ...
                        if (DBUS_TYPE_OBJECT_PATH == dbus_message_iter_get_arg_type(&variantArray)) {
                            ...
                            STRCPY_S(objectPath, sizeof(objectPath), path);
...
BtrCore_BTInitGetConnection (
    ...
    MEMSET_S(pstlhBtIfce->pcBTVersion, sizeof(char) * BT_MAX_STR_LEN, '\0', sizeof(char) * BT_MAX_STR_LEN);
    MEMSET_S(pstlhBtIfce->pcDeviceCurrState, sizeof(char) * BT_MAX_STR_LEN, '\0', sizeof(char) * BT_MAX_STR_LEN);
    MEMSET_S(pstlhBtIfce->pcLeDeviceCurrState, sizeof(char) * BT_MAX_STR_LEN, '\0', sizeof(char) * BT_MAX_STR_LEN);
    MEMSET_S(pstlhBtIfce->pcLeDeviceAddress, sizeof(char) * BT_MAX_STR_LEN, '\0', sizeof(char) * BT_MAX_STR_LEN);
    MEMSET_S(pstlhBtIfce->pcMediaCurrState, sizeof(char) * BT_MAX_STR_LEN, '\0', sizeof(char) * BT_MAX_STR_LEN);

    MEMSET_S(&pstlhBtIfce->lstBTAdapterInfo, sizeof(stBTAdapterInfo), 0, sizeof(stBTAdapterInfo));
    MEMSET_S(&pstlhBtIfce->lstBTDeviceInfo,  sizeof(stBTDeviceInfo), 0, sizeof(stBTDeviceInfo));
    MEMSET_S(&pstlhBtIfce->lstBTMediaInfo,  sizeof(stBTMediaInfo), 0, sizeof(stBTMediaInfo));

    MEMSET_S(&pstlhBtIfce->stBTLeGatt, sizeof(stBTLeGattInfo), 0, sizeof(stBTLeGattInfo));
    MEMSET_S(&pstlhBtIfce->stBTLeCustAdvt, sizeof(stBTLeCustomAdv), 0, sizeof(stBTLeCustomAdv));
    MEMSET_S(pstlhBtIfce->pcBTAdapterGattSrvEpPath, sizeof(char) * BT_MAX_DEV_PATH_LEN, '\0', sizeof(char) * BT_MAX_DEV_PATH_LEN);
...
BtrCore_BTUnregisterAgent (
     ...
     * Pairable true here */
    if (!strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_77, strlen(BT_BLUEZ_VERSION_5_77)) || !strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_55, strlen(BT_BLUEZ_VERSION_5_55)) || !strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_54, strlen(BT_BLUEZ_VERSION_5_54)) || !strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_48, strlen(BT_BLUEZ_VERSION_5_48)) )
    {
...
BtrCore_BTGetAdapterList (
    ...
    for (index = 0; index < BT_MAX_NUM_ADAPTERS; index++)
        MEMSET_S(&paths[index], BT_MAX_DEV_PATH_LEN, '\0',  BT_MAX_DEV_PATH_LEN);

    MEMSET_S(objectPath, BT_MAX_DEV_PATH_LEN, '\0', BT_MAX_DEV_PATH_LEN);
    MEMSET_S(objectData, BT_MAX_DEV_PATH_LEN, '\0', BT_MAX_DEV_PATH_LEN);
...
BtrCore_BTGetPairedDeviceInfo (
    ...
    for ( i = 0; i < num; i++) {
        MEMSET_S(&pPairedDeviceInfo->devicePath[i][0], BT_MAX_DEV_PATH_LEN, '\0', BT_MAX_DEV_PATH_LEN);
    ...
    for ( i = 0; i < num; i++) {
        ...
        if (lpDBusPendC != NULL) {
            ...
            if (lpDBusReply != NULL) {
                ...
                MEMSET_S(&apstBTDeviceInfo, sizeof(stBTDeviceInfo), 0, sizeof(stBTDeviceInfo));
                if (0 != btrCore_BTParseDevice(lpDBusReply, &apstBTDeviceInfo)) {
                    BTRCORELOG_ERROR ("Parsing the device %s failed..\n", pPairedDeviceInfo->devicePath[i]);
                    dbus_message_unref(lpDBusReply);
                    lpDBusReply = NULL;
    ...
    while (dbus_message_iter_get_arg_type(&element_i) != DBUS_TYPE_INVALID) {
        ...
        if (dbus_message_iter_get_arg_type(&element_i) == DBUS_TYPE_DICT_ENTRY) {
            ...
            if (apcDataString != NULL) {
                STRCPY_S(apcDataString, BT_MAX_STR_LEN*8, value); //TO DO: since size of destination varies
            }
...
BtrCore_BTPerformAdapterOp (
    ...
    switch (aenBTAdpOp) {
        ...
        STRCPY_S(deviceOpString,sizeof(deviceOpString), "FindDevice");
        break;
        case enBTAdpOpCreatePairedDev:
        case enBTAdpOpCreatePairedDevASync:
        STRCPY_S(deviceOpString,sizeof(deviceOpString), "Pair");
        break;
        case enBTAdpOpRemovePairedDev:
        STRCPY_S(deviceOpString,sizeof(deviceOpString), "RemoveDevice");
        break;
        default:
        rc = -1;
        break;
                                    ...
                                    while (dbus_message_iter_has_next(&innerArrayIter2)) {
                                        if (DBUS_TYPE_DICT_ENTRY == dbus_message_iter_get_arg_type(&innerArrayIter2)) {
                                            ...
                                            if (DBUS_TYPE_STRING == dbus_message_iter_get_arg_type(&innerDictEntryIter3)) {
                                                ...
                                                if (strcmp(apcDevPath, objectData) == 0) {
                                                    ++b;
                                                    STRCPY_S(deviceObjectPath,sizeof(deviceObjectPath),adapter_path);
                                                    adapterFound = TRUE;
                                                    break;
                                                }
                                            }
...
BtrCore_BTEnableEnhancedRetransmissionMode (
    ...
    MEMSET_S(lcpBtErtmIp, BT_MAX_STR_LEN/2, '\0', BT_MAX_STR_LEN/2);
    MEMSET_S(lcpBtErtmOp, BT_MAX_STR_LEN, '\0', BT_MAX_STR_LEN);
...
//sourced from linux/drivers/power/supply/power_supply_sysfs.c
static const char * const BTCORE_CAPACITY_LEVEL_TEXT[] = {
    "Unknown",
    "Critical",
    "Low",
    "Normal",
...
static const unsigned char BTCORE_CAPACITY_VALUES[] = 
{
    0, //UNKNOWN
    5, //CRITICAL
    20, //LOW
...
static unsigned char BtrCore_ConvertCapacityLevel(const char * pcBatteryLevelString)
{
    unsigned char i;
    for (i = 0; BTCORE_CAPACITY_LEVEL_TEXT[i] != NULL; i++)
    {
        if (!strncmp(BTCORE_CAPACITY_LEVEL_TEXT[i], pcBatteryLevelString, strlen(BTCORE_CAPACITY_LEVEL_TEXT[i])))
        {
            BTRCORELOG_ERROR("Converted string: %s into value %d\n",BTCORE_CAPACITY_LEVEL_TEXT[i], BTCORE_CAPACITY_VALUES[i]);
            return BTCORE_CAPACITY_VALUES[i];
        }
    }
    BTRCORELOG_ERROR("Capacity level string not found, returning 0 for UNKNOWN\n");
    return 0;

}
static int
BtrCore_BTGetBatteryFromHid(unsigned char * pDeviceBatteryLevel, char * pcMacAddress)
{
    struct udev *udev_context = NULL;
    struct udev_device *dev = NULL, *nintendo_dev = NULL;
    struct udev_enumerate *enumerate_context = NULL;
    struct udev_list_entry *battery_devices = NULL, *dev_list_entry = NULL;
    const char *path;
    char nintendo_device_path[MAX_NINTENDO_PATH_LEN] = {0};
    int i32OpRet  = -1;
    if (!pDeviceBatteryLevel || !pcMacAddress)
    {
        BTRCORELOG_ERROR("Either mac or battery lavel is null, could not get battery from power supply file\n");
        return -1;
    }
    udev_context = udev_new();
    if (!udev_context) {
        BTRCORELOG_ERROR("Cannot create udev context.\n");
        return -1;
    }
    enumerate_context = udev_enumerate_new(udev_context);
    if (!enumerate_context) {
        BTRCORELOG_ERROR("Cannot create enumerate context.\n");
        udev_unref(udev_context);
        return -1;
    }

    udev_enumerate_add_match_subsystem(enumerate_context, "power_supply");
    udev_enumerate_scan_devices(enumerate_context);

    battery_devices = udev_enumerate_get_list_entry(enumerate_context);
    if (!battery_devices) {
        BTRCORELOG_ERROR("Failed to get device list, no hid devices connected\n");
        udev_enumerate_unref(enumerate_context);
        udev_unref(udev_context);
        return -1;
    }
    udev_list_entry_foreach(dev_list_entry, battery_devices) {
        path = udev_list_entry_get_name(dev_list_entry);
        BTRCORELOG_TRACE("Checking path: %s", path);
        if (strstr(path, pcMacAddress) || strstr(path, "nintendo_switch_controller_battery"))
        {
            //if device is a nintendo we need to do further work to ensure it is the correct device
            if (strstr(path, "nintendo_switch_controller_battery"))
            {
                snprintf(nintendo_device_path, MAX_NINTENDO_PATH_LEN - 1, "%s%s", path, "/device/");
                //need to check if mac address matches in the power_supply/*/device folder
                BTRCORELOG_DEBUG("Querying device path: %s\n", nintendo_device_path);
                nintendo_dev = udev_device_new_from_syspath(udev_context, nintendo_device_path);
                if (!nintendo_dev)
                {
                    BTRCORELOG_ERROR("Could not create nintendo device from syspath\n");
                    continue;
                }
                if (udev_device_get_property_value(nintendo_dev, "HID_UNIQ"))
                {
                    if(!strstr(udev_device_get_property_value(nintendo_dev, "HID_UNIQ"), pcMacAddress))
                    {
                        BTRCORELOG_INFO("Nintendo device is not the intended target\n");
                        udev_device_unref(nintendo_dev);
                        continue;
                    }
                    BTRCORELOG_INFO("Nintendo device is the intended target\n");
                    udev_device_unref(nintendo_dev);
                }
            }
            dev = udev_device_new_from_syspath(udev_context, path);
            if (udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY_LEVEL"))
            {
                BTRCORELOG_INFO("Reading from POWER_SUPPLY_CAPACITY_LEVEL, battery level: %s\n", udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY_LEVEL"));
                *pDeviceBatteryLevel = BtrCore_ConvertCapacityLevel(udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY_LEVEL"));
                i32OpRet = 0;
            }
            else if (udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY"))
            {
                BTRCORELOG_INFO("Reading from POWER_SUPPLY_CAPACITY, battery level: %s\n", udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY"));
                *pDeviceBatteryLevel = strtol(udev_device_get_property_value(dev, "POWER_SUPPLY_CAPACITY"),NULL, 10);
                i32OpRet = 0;
            }
            else
            {
                BTRCORELOG_ERROR("Could not get capacity from %s\n", path);
                i32OpRet = -1;
            }
    ...
    BTRCORELOG_ERROR("Requested device not found\n");
    udev_enumerate_unref(enumerate_context);
    udev_unref(udev_context);
    return -1;
}
...
)
{
    ...
    int                 dbus_type = DBUS_TYPE_INVALID;
    int                 propertyValue;
    char                pcMacAddress[MAC_ADDRESS_LENGTH];
    

    const char * pDeviceInterface = BT_DBUS_BLUEZ_BATTERY_PATH;
    const char * pDeviceCommand = "Percentage";
    if (!apstBtIfceHdl || !lpcBTRCoreBTDevicePath || !pDeviceBatteryLevel )
    {
        return -1;
    }
    //try and read battery level for path
    lpDBusMsg = dbus_message_new_method_call(BT_DBUS_BLUEZ_PATH,
                                             lpcBTRCoreBTDevicePath,
                                             DBUS_INTERFACE_PROPERTIES,
                                             "Get");

    dbus_message_iter_init_append(lpDBusMsg, &args);
    dbus_message_append_args(lpDBusMsg, DBUS_TYPE_STRING, &pDeviceInterface, DBUS_TYPE_INVALID);
    dbus_message_append_args(lpDBusMsg, DBUS_TYPE_STRING, &pDeviceCommand, DBUS_TYPE_INVALID);
    ...
    if (!lpDBusReply) {
        BTRCORELOG_ERROR ("lpDBusReply Null\n");
        btrCore_BTHandleDusError(&lDBusErr, __LINE__, __FUNCTION__);
        //no battery information from bluez attempt to read from power_supply
        //get pcMacAddres from path
        sscanf(lpcBTRCoreBTDevicePath, "/org/bluez/hci0/dev_%2s_%2s_%2s_%2s_%2s_%2s", &pcMacAddress[0],
                                                              &pcMacAddress[3],
                                                              &pcMacAddress[6],
                                                              &pcMacAddress[9],
                                                              &pcMacAddress[12],
                                                              &pcMacAddress[15] );
        for (unsigned char i = 2; i < MAC_ADDRESS_LENGTH - 3; i+=3)
            pcMacAddress[i] = ':';
        for (unsigned char i = 0; i < MAC_ADDRESS_LENGTH; i++)
            pcMacAddress[i] = tolower(pcMacAddress[i]);
        if(!BtrCore_BTGetBatteryFromHid(pDeviceBatteryLevel, pcMacAddress))
        {
            BTRCORELOG_INFO("Battery level read from power_supply subsystem\n");
            return 0;
        }
        else {
            BTRCORELOG_ERROR("Could not get battery level from bluez or power_supply subsystem, is device connected?\n");
   ...
   dbus_connection_flush(pstlhBtIfce->pDBusConn);
    ...
    dbus_message_unref(lpDBusReply);
    *pDeviceBatteryLevel = propertyValue;
    return 0;
}
int
...
BtrCore_BTUnRegisterMedia (
    ...
    case enBTDevAudioSink:
        ...
        // CID 23347: Logically dead code (DEADCODE)
        break;
    case enBTDevAudioSource:
        if (lBtMediaCodec == BT_MEDIA_CODEC_SBC) {
            lpBtMediaEpObjPath = BT_MEDIA_SBC_A2DP_SINK_ENDPOINT;
...
BtrCore_BTSetDevDataAckTimeout (
    ...
    FILE*           lHciConList = NULL;
    int             i32OpRet    = -1;
    char            lcpHciDataWriteAckTOutIp[BT_MAX_STR_LEN/2] = {'\0'};
    char            lcpHciDataWriteAckTOutOp[BT_MAX_STR_LEN] = {'\0'};
    char            lcpHciConLine[BT_MAX_STR_LEN/2] = {'\0'};
    char *          lcpHciConLineTarget = NULL;
    char            mac[18];
    char            connectedMac[18];
    uint16_t        handle, deviceHandle = 0xFFFFu;

    if (!apstBtIfceHdl)
        return -1;

    (void)pstlhBtIfce;
    if (pstlhBtIfce->pcDevTransportPath == NULL)
    {
        BTRCORELOG_ERROR("pcDevTransportPath is currently NULL so could not get mac address\n");
        return -1; 
    }
    btrCore_BTGetDevAddressFromDevPath(pstlhBtIfce->pcDevTransportPath, connectedMac);
    // get connection handle for current device
    lHciConList = popen("hcitool con", "r");
    if (lHciConList != NULL)
    {
        //get "Connected:" line
        fgets(lcpHciConLine, BT_MAX_STR_LEN/2, lHciConList);
        //        < ACL 00:1B:66:A6:71:B8 handle 11 state 1 lm SLAVE AUTH ENCRYPT
        while(fgets(lcpHciConLine, BT_MAX_STR_LEN/2, lHciConList) != NULL)
        {
            lcpHciConLineTarget = strstr(lcpHciConLine, " ACL");
            if (lcpHciConLineTarget) {
                if (sscanf(lcpHciConLineTarget, " ACL %17s handle %hd", mac, &handle) != 2)
                {
                    BTRCORELOG_ERROR("Failed to read one or more parameters from hcitool con\n");
                    pclose(lHciConList); // CID 337636 : Resource leak (RESOURCE_LEAK)
                    return -1;
                }
                BTRCORELOG_DEBUG("MAC address: %s, handle: %d\n", mac, handle);
                if (strncmp(mac, connectedMac, 18)==0)
                {
                    BTRCORELOG_INFO("Device handle found: %d\n", handle);
                    deviceHandle = handle;
                    break;
                }
            }
    ...
    {
        BTRCORELOG_ERROR("Could not run hcitool con\n");
        return -1;
    }

    if (deviceHandle == 0xFFFFu)
    {
        BTRCORELOG_ERROR("Could not find device in hcitool con");
        pclose(lHciConList); // CID 337636 : Resource leak (RESOURCE_LEAK)
        return -1;
    }
    pclose(lHciConList);
    ...
    snprintf(lcpHciDataWriteAckTOutIp, BT_MAX_STR_LEN/2, "hcitool -i hci0 cmd 0x03 0x0028 0x%02x 0x%02x 0x%02x 0x00", deviceHandle & 0x00FF, deviceHandle >> 8, aui32AckTOutms);
...
int
BtrCore_BTDevMediaControlVolume (
   void*               apstBtIfceHdl,
   const char*         apMediaControlPath,
   enBTMediaControlCmd aenBTMediaOper
    ...
    if (!apstBtIfceHdl || !apMediaControlPath) {
        BTRCORELOG_ERROR ("Invalid args!!!");
        return -1;
    }
    ...
    switch (aenBTMediaOper) {
        case enBTMediaCtrlVolumeUp:
            safec_rc = strcpy_s(mediaOper, sizeof(mediaOper), "VolumeUp"); // CID 347092: Calling risky function (DC.STRING_BUFFER)
            ERR_CHK(safec_rc);
            break;
        case enBTMediaCtrlVolumeDown:
            safec_rc = strcpy_s(mediaOper, sizeof(mediaOper), "VolumeDown");
            ERR_CHK(safec_rc);
            break;
        default:
            BTRCORELOG_INFO("Invalid Methods ...\n");
            break;
    }

    if (mediaOper[0]) {
        BTRCORELOG_INFO("Invoking volume up/down method call ...\n");
        lpDBusMsg = dbus_message_new_method_call(BT_DBUS_BLUEZ_PATH,
                apMediaControlPath,
...
BtrCore_BTRegisterLeAdvertisement (
    ...
    MEMSET_S(pstlhBtIfce->stBTLeCustAdvt.pui8AdvertPath, BT_MAX_STR_LEN, '\0', BT_MAX_STR_LEN);
    strncpy(pstlhBtIfce->stBTLeCustAdvt.pui8AdvertPath, apBtAdapter, BT_MAX_STR_LEN - 1);
...
int
BtrCore_BTReleaseAdvertisement (
    void*       apstBtIfceHdl,
    const char* apBtAdapter
) {
...
BtrCore_BTPerformLeGattOp (
    ...
    case enBTLeGattOpReadValue:
        STRCPY_S(lcBtGattOpString,sizeof(lcBtGattOpString), "ReadValue");
        break;
    case enBTLeGattOpWriteValue:
        STRCPY_S(lcBtGattOpString,sizeof(lcBtGattOpString), "WriteValue");
        break;
    case enBTLeGattOpStartNotify:
        STRCPY_S(lcBtGattOpString,sizeof(lcBtGattOpString), "StartNotify");
        break;
    case enBTLeGattOpStopNotify:
        STRCPY_S(lcBtGattOpString,sizeof(lcBtGattOpString), "StopNotify");
        break;
    ...
    if (aenBTLeGattOp == enBTLeGattOpWriteValue && apLeGatOparg2) {
        ...
        MEMSET_S(lpcLeWriteByteData, BT_MAX_STR_LEN, 0, BT_MAX_STR_LEN);
    ...
    if (aenBTLeGattOp == enBTLeGattOpReadValue) {
       ...
       MEMSET_S(byteValue, sizeof(byteValue), 0, sizeof(byteValue));
       ...
       byteValue[u16idx] = '\0';
       safec_rc = strcpy_s (rpLeOpRes, BT_MAX_STR_LEN-1, byteValue); // CID 23374: Logically dead code (DEADCODE)
       ERR_CHK(safec_rc);
...
BtrCore_BTSendReceiveMessages (
    ...
    if (pstlhBtIfce->pDBusConn == NULL || (dbus_connection_get_is_connected(pstlhBtIfce->pDBusConn) != TRUE))
        return -1;
...
btrCore_BTDBusConnectionFilterCb (
    ...
    MEMSET_S(pstBTAdapterInfo, sizeof(stBTAdapterInfo), 0, sizeof(stBTAdapterInfo));
    MEMSET_S(pstBTDeviceInfo, sizeof(stBTDeviceInfo), 0, sizeof(stBTDeviceInfo));
    MEMSET_S(pstBTMediaInfo, sizeof(stBTMediaInfo), 0, sizeof(stBTMediaInfo));
    ...
    if (li32MessageType == DBUS_MESSAGE_TYPE_ERROR) {
            ...
            if (lpcDBusIface) {
                if (!strcmp(lpcDBusIface, BT_DBUS_BLUEZ_ADAPTER_PATH)) {
                    ...
                    if (dbus_message_iter_get_arg_type(&lDBusMsgIterDict) == DBUS_TYPE_INVALID) {
                        settingRSSItoZero = 1;
                        BTRCORELOG_DEBUG ("Setting Dev %s RSSI to 0...\n", pstBTDeviceInfo->pcAddress);
                    }
                    else {
                        DBusMessageIter lDBusMsgParse;

                        BTRCORELOG_INFO ("Device MAC address %s\n", pstBTDeviceInfo->pcAddress);
                        while (dbus_message_iter_get_arg_type(&lDBusMsgIterDict) == DBUS_TYPE_DICT_ENTRY) {
                            ...
                            if (dbus_message_iter_get_arg_type(&lDBusMsgParse) == DBUS_TYPE_STRING) {
                                ...
                                if (strcmp (pNameOfProperty, "Paired") == 0) {
                                    ...
                                    pstBTDeviceInfo->bPaired = bPaired;
                                    bPairingEvent = 1;
                                ...
                                else if (strcmp (pNameOfProperty, "UUIDs") == 0) {
                                    DBusMessageIter variant_i;
                                    int dbus_type;
                                    ...
                                    if (dbus_type == DBUS_TYPE_ARRAY) {
                                        ...
                                        while ((dbus_type = dbus_message_iter_get_arg_type (&variant_j)) != DBUS_TYPE_INVALID) {
                                            if ((dbus_type == DBUS_TYPE_STRING) && (count < BT_MAX_DEVICE_PROFILE)) {
                                                ...
                                                dbus_message_iter_get_basic (&variant_j, &pVal);
                                                BTRCORELOG_TRACE ("UUID value is %s\n", pVal);
                                                strncpy(pstBTDeviceInfo->aUUIDs[count], pVal, (BT_MAX_UUID_STR_LEN - 1));
                                                count++;
                                            }
                                            dbus_message_iter_next (&variant_j);
                                        }
                                ...
                                {
                                    bClassOrAppearanceEvent = 1;
                                    dbus_message_iter_next(&lDBusMsgParse);
                                    dbus_message_iter_recurse(&lDBusMsgParse, &lDBusMsgPropertyValue);
                                    dbus_message_iter_get_basic(&lDBusMsgPropertyValue, &ui16Appearance);
                                    BTRCORELOG_INFO("Received new Appearance %hd", ui16Appearance);
                                }
                                else if (strcmp (pNameOfProperty, "Class") == 0)
                                {
                                    bClassOrAppearanceEvent  = 1;
                                    dbus_message_iter_next(&lDBusMsgParse);
                                    dbus_message_iter_recurse(&lDBusMsgParse, &lDBusMsgPropertyValue);
                    ...
                    if (!i32OpRet) {
                        ...
                        if (bPairingEvent) {
                            ...
                            else if (pstlhBtIfce->ui32IsAdapterDiscovering && settingRSSItoZero) {
                                lenBtDevState = enBTDevStRSSIUpdate;
                            }
                            else {
                        ...
                        else if ((lenBTDevType == enBTDevHID)) {
                            if (bConnectEvent) {
                                if (pstBTDeviceInfo->bConnected) {
                                    const char* value = "connected";

                                    strncpy(pstBTDeviceInfo->pcDevicePrevState, pstBTDeviceInfo->pcDeviceCurrState, BT_MAX_STR_LEN - 1);
                                    strncpy(pstBTDeviceInfo->pcDeviceCurrState, value, BT_MAX_STR_LEN - 1);
                                }
                                else if (!pstBTDeviceInfo->bConnected) {
                                    const char* value = "disconnected";
                    ...
                    if (!strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_77, strlen(BT_BLUEZ_VERSION_5_77)) || !strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_55, strlen(BT_BLUEZ_VERSION_5_55)) || !strncmp(pstlhBtIfce->pcBTVersion, BT_BLUEZ_VERSION_5_54, strlen(BT_BLUEZ_VERSION_5_54))) {
                    ...
                    case enBTMedTPropState:
		        BTRCORELOG_INFO  ("Current streaming states: pcState:%s  pcMediaCurrState:%s\n",pcState, pstlhBtIfce->pcMediaCurrState );
                        if ((!strcmp(pstlhBtIfce->pcMediaCurrState, "none")) && (!strcmp(pcState, "pending"))) {
                            STRCPY_S(pstlhBtIfce->pcMediaCurrState,sizeof(pstlhBtIfce->pcMediaCurrState), pcState);
                            ...
                            if (!i32OpRet && pstBTDeviceInfo->bConnected && pstBTDeviceInfo->bPaired) {
                                ...
                                else if ((!strcmp(pstlhBtIfce->pcMediaCurrState, "pending") || !strcmp(pstlhBtIfce->pcMediaCurrState, "active")) && (!strcmp(pcState, "idle"))) {
                                    
                                    const char* value = "suspended";
                                    enBTDeviceState lenBtDevState = enBTDevStPropChanged;

                                    BTRCORELOG_ERROR("External device has suspended AVDTP stream\n");
                                    strncpy(pstBTDeviceInfo->pcDevicePrevState, pstlhBtIfce->pcDeviceCurrState, BT_MAX_STR_LEN - 1);
                                    strncpy(pstBTDeviceInfo->pcDeviceCurrState, value, BT_MAX_STR_LEN - 1);
                                    strncpy(pstlhBtIfce->pcDeviceCurrState, value, BT_MAX_STR_LEN - 1);
                    ...
                    case enBTMedTPropDelay:
                        {
                            stBTMediaStatusUpdate mediaStatusUpdate;
                            BTRCORELOG_INFO ("MediaTransport Property - Delay: %u\n", lDelay);
                            mediaStatusUpdate.aenBtOpIfceType                        = enBTMediaTransport;
                            mediaStatusUpdate.aunBtOpIfceProp.enBtMediaTransportProp = lenBTMedTransProp;
                            mediaStatusUpdate.m_mediaTransportDelay                  = lDelay;
                            ...
                            if (dbus_message_iter_get_arg_type(&lDBusMsgIter1) == DBUS_TYPE_STRING) {
                                ...
                                */
                                if (!strncmp(str, "Connected", strlen("Connected"))) {
                                    ...
                                    MEMSET_S(mediaStatusUpdate.m_mediaPlayerPath, BT_MAX_STR_LEN, 0, BT_MAX_STR_LEN);
                    ...
                    MEMSET_S(&mediaStatusUpdate, sizeof(stBTMediaStatusUpdate), 0, sizeof(stBTMediaStatusUpdate));
                            ...
                            if (dbus_message_iter_get_arg_type(&lDBusMsgIter1) == DBUS_TYPE_STRING) {
                                ...
                                */
                                if (!strncmp(str, "Name", strlen("Name"))) {
                                    ...
                                    MEMSET_S(&mediaTrackInfo, sizeof(stBTMediaTrackInfo), 0, sizeof(stBTMediaTrackInfo));
                                    ...
                                    if (!BtrCore_BTGetTrackInformation (pstlhBtIfce, apcMediaPlayerIface, &mediaTrackInfo)) {
                                        ...
                                        MEMCPY_S(&mediaStatusUpdate.m_mediaTrackInfo,sizeof(mediaStatusUpdate.m_mediaTrackInfo), &mediaTrackInfo, sizeof(stBTMediaTrackInfo));
                                    }
                                }
                            ...
                            if (dbus_message_iter_get_arg_type(&lDBusMsgIter1) == DBUS_TYPE_STRING) {
                                ...
                                */
                                if (!strncmp(str, "Playable", strlen("Playable"))) {
                                    ...
                                    stBTMediaTrackInfo mediaTrackInfo;
                                    MEMSET_S(mediaPlayerIface, BT_MAX_STR_LEN, 0, BT_MAX_STR_LEN);
                                    MEMSET_S(&mediaTrackInfo, sizeof(stBTMediaTrackInfo), 0, sizeof(stBTMediaTrackInfo));
        ...
        if (dbus_message_iter_get_arg_type(&lDBusMsgIter) == DBUS_TYPE_OBJECT_PATH) {
            ...
            if (dbus_message_iter_get_arg_type(&lDBusMsgIter) == DBUS_TYPE_ARRAY) {
                BTRCORELOG_DEBUG ("InterfacesAdded : Interface %s\n", lpcDBusIface ? lpcDBusIface : NULL);
                        ...
                        if (dbus_message_iter_get_arg_type(&lDBusMsgIterStrnArr) == DBUS_TYPE_ARRAY) {
                            if (lpcDBusIfaceInternal) {
                                if (!strcmp(lpcDBusIfaceInternal, BT_DBUS_BLUEZ_ADAPTER_PATH)) {
                                    BTRCORELOG_DEBUG ("InterfacesAdded : %s\n", BT_DBUS_BLUEZ_ADAPTER_PATH);
                                }
                                    ...
                                    if (lpcDBusIface) {
                                        ui32DeviceIfceLen = strstr(lpcDBusIface, "/player") - lpcDBusIface;
                                    }
                                    ...
                                    if ((lpcDBusIface) && (strstr(lpcDBusIface, "NowPlaying"))) {
                                        stBTMediaBrowserUpdate mediaBrowserUpdate;
                                        MEMSET_S(&mediaBrowserUpdate, sizeof(stBTMediaBrowserUpdate), 0, sizeof(stBTMediaBrowserUpdate)); //CID:163772 - Uninitialized scalar variable
                                        ...
                                        if (strstr(lpcDBusIface, "item")) {
                                            ...
                                            if (/*!i32OpRet &&*/ !(i32OpRet = BtrCore_BTGetTrackInformation (pstlhBtIfce, apcMediaIfce, &mediaTrackInfo))) {
                                                ...
                                                if (!strncmp(mediaTrackInfo.pcTitle, name, (strlen(mediaTrackInfo.pcTitle) < strlen(name))?
                                                    ...
                                                    MEMCPY_S(&mediaBrowserUpdate.mediaTrackInfo,sizeof(mediaBrowserUpdate.mediaTrackInfo), &mediaTrackInfo, sizeof(stBTMediaTrackInfo));
                                                }
                                                /*else {
                                    ...
                                    else if ((lpcDBusIface) && (strstr(lpcDBusIface, "Filesystem"))) {
                                        stBTMediaBrowserUpdate mediaBrowserUpdate;
                                        ...
                                        MEMSET_S(&mediaBrowserUpdate, sizeof(stBTMediaBrowserUpdate), 0, sizeof(stBTMediaBrowserUpdate));
                                ...
                                /* Add Interfaces for GATT */
                                    ...
                                    unsigned int ui32DeviceIfceLen = 0;
                                    if(lpcDBusIface) {
                                       ui32DeviceIfceLen = strstr(lpcDBusIface, "/service") - lpcDBusIface; 
                                    }
        ...
        if (dbus_message_iter_get_arg_type(&lDBusMsgIter) == DBUS_TYPE_ARRAY) {
            BTRCORELOG_DEBUG ("InterfacesRemoved : Interface %s\n", lpcDBusIface ? lpcDBusIface : NULL);
            ...
            while (dbus_message_iter_get_arg_type(&lDBusMsgIterStr) == DBUS_TYPE_STRING) {
                ...
                if (lpcDBusIfaceInternal) {
                    if (!strcmp(lpcDBusIfaceInternal, BT_DBUS_BLUEZ_ADAPTER_PATH)) {
                        BTRCORELOG_DEBUG ("InterfacesRemoved : %s\n", BT_DBUS_BLUEZ_ADAPTER_PATH);
                    }
                    else if (!strcmp(lpcDBusIfaceInternal, BT_DBUS_BLUEZ_DEVICE_PATH)) {
                        BTRCORELOG_DEBUG ("InterfacesRemoved : %s\n", BT_DBUS_BLUEZ_DEVICE_PATH);
...
btrCore_BTLeGattEndpointHandlerCb (
    ...
    if (dbus_message_is_method_call(apDBusMsg, DBUS_INTERFACE_OBJECT_MANAGER, "GetManagedObjects")) {
        ...
        if (!strncmp(lpcPath, BT_LE_GATT_SERVER_ENDPOINT, strlen(BT_LE_GATT_SERVER_ENDPOINT)) ||
            !strncmp(lpcPath, pstlhBtIfce->pcBTAdapterGattSrvEpPath, strlen(pstlhBtIfce->pcBTAdapterGattSrvEpPath))) {
