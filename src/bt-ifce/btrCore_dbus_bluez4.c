...
btrCore_BTAgentRequestConfirmation (
    ...
    if (gfpcBConnectionIntimation && lpcPath) {
        ...
        yesNo = gfpcBConnectionIntimation(enBTDevUnknown, &lstBTDeviceInfo, ui32PassCode, 1, gpcBConnIntimUserData);
    }
...
btrCore_BTMediaEndpointSelectConfiguration (
    ...
    if (gfpcBNegotiateMedia) {
        if(gfpcBNegotiateMedia(lpInputMediaCaps, &lpOutputMediaCaps, 0, 0, gpcBNegMediaUserData)) {
            return dbus_message_new_error(apDBusMsg, "org.bluez.MediaEndpoint.Error.InvalidArguments", "Unable to select configuration");
        }
    }
...
btrCore_BTMediaEndpointClearConfiguration (
    ...
    if (gfpcBTransportPathMedia) {
        if(!gfpcBTransportPathMedia(lDevTransportPath, NULL, NULL, enBTDevAudioSink, enBTMediaTypeSBC, gpcBTransPathMediaUserData)) {
...
BtrCore_BTGetPairedDeviceInfo (
    ...
    MEMSET_S(pPairedDeviceInfo, sizeof (stBTPairedDeviceInfo), 0, sizeof (stBTPairedDeviceInfo));
    ...
    if (lpDBusReply != NULL) {
        ...
        for ( i = 0; i < num; i++) {
            STRCPY_S(pPairedDeviceInfo->devicePath[i],sizeof(pPairedDeviceInfo->devicePath[0]), paths[i]);
        }
        dbus_message_unref(lpDBusReply);
    }
    ...
    for ( i = 0; i < num; i++) {
        ...
        if (lpDBusReply != NULL) {
            MEMSET_S(&apstBTDeviceInfo, sizeof(apstBTDeviceInfo), 0, sizeof(apstBTDeviceInfo));
            if (0 != btrCore_BTParseDevice(lpDBusReply, &apstBTDeviceInfo)) {
                ...
                MEMCPY_S(&pPairedDeviceInfo->deviceInfo[i],sizeof(pPairedDeviceInfo->deviceInfo[0]), &apstBTDeviceInfo, sizeof(apstBTDeviceInfo));
            }
        }
        dbus_message_unref(lpDBusReply);
    }
...
BtrCore_BTGetPairedDevices (
    ...
    for ( i = 0; i < num; i++) {
        if (apcArrPairedDevPath[i]) {
            STRCPY_S(apcArrPairedDevPath[i],sizeof(apcArrPairedDevPath[0]), paths[i]);
            rc = 0;
        }
    }
    ...
    while (dbus_message_iter_get_arg_type(&element_i) != DBUS_TYPE_INVALID) {
        ...
        if (dbus_message_iter_get_arg_type(&element_i) == DBUS_TYPE_DICT_ENTRY) {
            ...
            if (apcDataString != NULL) {
                strcpy(apcDataString, value); //TODO: Since apcDataString has different size
            }
...
BtrCore_BTPerformAdapterOp (
    ...
    switch (aenBTAdpOp) {
        ...
        case enBTAdpOpCreatePairedDev:
            STRCPY_S(deviceOpString,sizeof(deviceOpString), "CreatePairedDevice");
            break;
...
BtrCore_BTRegisterMedia (
    ...
    }
    {
        ...
        char array_type[6] = "a";
        strncat (array_type, (char*)&type, sizeof(array_type) - 1);
