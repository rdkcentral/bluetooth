...
#if defined(USE_BLUEZ5) || defined(USE_GDBUSBLUEZ5)
...
btrCore_AVMedia_GetA2DPDefaultBitpool (
    ...
    case BTR_SBC_SAMPLING_FREQ_44100:
        ...
        case BTR_SBC_CHANNEL_MODE_STEREO:
        case BTR_SBC_CHANNEL_MODE_JOINT_STEREO:
            return BTR_SBC_DEFAULT_BITRATE_BITPOOL_441;
...
btrCore_AVMedia_AllocateBrowserMemory (
    ...
    MEMSET_S(*apstBrowserNew, sizeof(stBTRCoreAVMediaItem), 0, sizeof(stBTRCoreAVMediaItem));
...
btrCore_AVMedia_DeallocateBrowserMemory (
    ...
    if ((ptr = (*apstBrowser)->pvAVMediaParentItem)) {
        if (ptr->pstAVMediaSubItems[ptr->ui32AVMediaNumberOfItems -1]->ui32AVMediaItemId != (*apstBrowser)->ui32AVMediaItemId) {
            MEMCPY_S(*apstBrowser,sizeof(stBTRCoreAVMediaItem), ptr->pstAVMediaSubItems[ptr->ui32AVMediaNumberOfItems -1], sizeof(stBTRCoreAVMediaItem));
            *apstBrowser = ptr->pstAVMediaSubItems[ptr->ui32AVMediaNumberOfItems -1];
        }
    }

    MEMSET_S(*apstBrowser, sizeof(stBTRCoreAVMediaItem), 0, sizeof(stBTRCoreAVMediaItem));
...
btrCore_AVMedia_DeallocateUnhandledBrowserMemory (
    ...
    while (apstBrowser) {
        ...
        free ((void*)ptr);
        //TODO: CID 44524 Out-of-bounds write (OVERRUN)
        if (ui8LoopIdx > 0) // CID 44524 Out-of-bounds write (OVERRUN)
...
btrCore_AVMedia_SwitchToMediaBrowserItem (
    ...
    if ((curr = apstlhBTRCoreAVM->pstAVMediaBrowser)) {
        ...
        MEMSET_S(mediaItemPath, 15 * BTRCORE_MAX_STR_LEN, 0, 15 * BTRCORE_MAX_STR_LEN);
        ...
        if (comnItemId != curr->ui32AVMediaItemId) {
            ...
            while (curr->pvAVMediaParentItem) {
                ...
                if (comnItemId != dest->ui32AVMediaItemId || curr->ui32AVMediaItemId != comnItemId) {
                    safec_rc = strcpy_s (mediaItemPath[countIdx], BTRCORE_MAX_STR_LEN -1, curr->pcAVMediaItemPath); // CID 342542: String not null terminated (STRING_NULL)
                    countIdx++;
                    ERR_CHK(safec_rc);
                }
        ...
        if (comnItemId != dest->ui32AVMediaItemId) {
            ...
            while (dest->pvAVMediaParentItem) {
                ...
                if (comnItemId != dest->ui32AVMediaItemId) {
                    safec_rc = strcpy_s (mediaItemPath[countIdx], BTRCORE_MAX_STR_LEN -1, dest->pcAVMediaItemPath);
                    countIdx++;
                    ERR_CHK(safec_rc);
                }
                else {
...
btrCore_AVMedia_PlaybackPositionPolling (
    ...
    mediaStatus.eAVMediaState = eBTRCoreAVMediaStUnknown;
...
BTRCore_AVMedia_Init (
    ...
    MEMSET_S(pstlhBTRCoreAVM, sizeof(stBTRCoreAVMediaHdl),  0, sizeof(stBTRCoreAVMediaHdl));
    ...
    pstlhBTRCoreAVM->i64LastSelectConfigTime        = 0;
    ...
    memset(&lstBtA2dpSbcCaps, 0, sizeof(a2dp_sbc_t));
    ...
    MEMCPY_S(pstlhBTRCoreAVM->pstBTMediaConfigOut, MAX(MAX(sizeof(a2dp_aac_t), sizeof(a2dp_mpeg_t)), sizeof(a2dp_sbc_t)), &lstBtA2dpSbcCaps, sizeof(a2dp_sbc_t));
    ...
    MEMCPY_S(pstlhBTRCoreAVM->pstBTMediaConfigIn, MAX(MAX(sizeof(a2dp_aac_t), sizeof(a2dp_mpeg_t)), sizeof(a2dp_sbc_t)), &lstBtA2dpSbcCaps, sizeof(a2dp_sbc_t));
  ...
  lBtAVMediaASrcRegRet = 0;
  BTRCORELOG_INFO("Streaming in from bluetooth device is not supported\n");
...
BTRCore_AVMedia_AcquireDataPath (
    ...
    if (BtrCore_BTAcquireDevDataPath(pstlhBTRCoreAVM->btIfceHdl, lpcAVMediaTransportPath, apDataPath, apDataReadMTU, apDataWriteMTU)) {
        BTRCORELOG_ERROR ("Failed to ReAcquire transport path %s\n", lpcAVMediaTransportPath);
     }
     else {
    ...
    lBtAVMediaRet = BtrCore_BTGetProp(pstlhBTRCoreAVM->btIfceHdl, lpcAVMediaTransportPath, enBTMediaTransport, lunBtOpMedTProp, &ui32Delay);
    if(lBtAVMediaRet)
       lenBTRCoreRet = enBTRCoreFailure;

    lunBtOpMedTProp.enBtMediaTransportProp = enBTMedTPropVol;
    lBtAVMediaRet = BtrCore_BTGetProp(pstlhBTRCoreAVM->btIfceHdl, lpcAVMediaTransportPath, enBTMediaTransport, lunBtOpMedTProp, &ui16Vol);
    if(lBtAVMediaRet)
       lenBTRCoreRet = enBTRCoreFailure;
...
BTRCore_AVMedia_ReleaseDataPath (
    ...
    lBtAVMediaRet = BtrCore_BTReleaseDevDataPath(pstlhBTRCoreAVM->btIfceHdl, lpcAVMediaTransportPath);
    if(!lBtAVMediaRet)
       lenBTRCoreRet = enBTRCoreSuccess;
...
BTRCore_AVMedia_MediaControl (
    ...
    if (aenBTMediaControl == enBTMediaCtrlUnknown) {
        ...
        if (((pstlhBTRCoreAVM->bAVMediaTrVolAvrcp == TRUE) || (pstlhBTRCoreAVM->bAVMediaPlayerConnected == TRUE)) && (pstlhBTRCoreAVM->pcAVMediaTransportPathOut != NULL)) {
            BTRCORELOG_INFO("Performing AV MediaCtrl operation Volume - %d\n",pstlhBTRCoreAVM->ui8AVMediaTransportVolume);
            if ((pcDeviceName != NULL) && ((!strncmp("Philips TAB7305",pcDeviceName,strlen("Philips TAB7305"))) || (!strncmp("DHT-S216",pcDeviceName,strlen("DHT-S216")))) && (aenBTMediaControl == enBTMediaCtrlVolumeUp || aenBTMediaControl == enBTMediaCtrlVolumeDown)) {
                BTRCORELOG_INFO("Volume change for philips device \n");
                lBtAVMediaRet = BtrCore_BTDevMediaControlVolume(pstlhBTRCoreAVM->btIfceHdl, apBtDevAddr, aenBTMediaControl);
            } else {
                lunBtOpMedTProp.enBtMediaTransportProp = enBTMedTPropVol;
                BTRCORELOG_INFO("Setting Volume property on MediaTransport1\n");
                lBtAVMediaRet = BtrCore_BTSetProp(pstlhBTRCoreAVM->btIfceHdl, pstlhBTRCoreAVM->pcAVMediaTransportPathOut, enBTMediaTransport, lunBtOpMedTProp, &lui16TranspVol);
            }
            if (lBtAVMediaRet != 0) {
                lenBTRCoreRet = enBTRCoreFailure;
            } else {
...
BTRCore_AVMedia_GetElementTrackInfo (
    ...
    if (aBtrAVMediaItemId && (pstlhBTRCoreAVM->SelectedaBtrAVMediaItemId == aBtrAVMediaItemId)) {
        if (apstBTAVMediaTrackInfo) {
            MEMSET_S(apstBTAVMediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo), 0 ,sizeof(stBTRCoreAVMediaTrackInfo));
            MEMCPY_S(apstBTAVMediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo),&pstlhBTRCoreAVM->SelectedapstBTAVMediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo));
...
BTRCore_AVMedia_GetMediaElementList (
    ...
    if (lpstBtAVMediaItem->pstAVMediaSubItems) {
        ...
        MEMSET_S(aAVMediaElementInfoList, sizeof(stBTRCoreAVMediaElementInfoList), 0, sizeof(stBTRCoreAVMediaElementInfoList));
        ...
        while (aui16StartIdx <= aui16EndIdx && aui16StartIdx < lpstBtAVMediaItem->ui32AVMediaNumberOfItems && lpstBtAVMediaItem->pstAVMediaSubItems[aui16StartIdx]) {
            ...
            if ((aeBtrAVMElementType == eBTRCoreAVMETypeUnknown) || (aeBtrAVMElementType == lpstBtAVMediaItem->pstAVMediaSubItems[aui16StartIdx]->eMediaItemType)) {
                ...
                MEMCPY_S(&aAVMediaElementInfoList->m_mediaElementInfo[m_numOfElements].m_mediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo),&lpstBtAVMediaItem->pstAVMediaSubItems[aui16StartIdx]->mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo)); // CID 330350, 330347: Evaluation order violation (EVALUATION_ORDER)
                m_numOfElements++;
                aui16StartIdx++;
            }
            else {
        ...
        if (ptrTrack->bIsMediaItemPlayable) {
            if (ptrTrack->pcAVMediaItemPath[0]) {
                                     ...
                                     ptrTrack->pcAVMediaItemPath, (stBTMediaTrackInfo*)&apstBTAVMediaTrackInfo))
                {
                    ...
                    MEMSET_S(&pstlhBTRCoreAVM->SelectedapstBTAVMediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo), 0, sizeof(stBTRCoreAVMediaTrackInfo));
                    MEMCPY_S(&pstlhBTRCoreAVM->SelectedapstBTAVMediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo), &apstBTAVMediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo));
...
btrCore_AVMedia_NegotiateMediaCb (
    ...
    if (pstlhBTRCoreAVM->eAVMediaTypeOut == eBTRCoreAVMTypeSBC &&
    pstlhBTRCoreAVM->pcAVMediaTransportPathOut == NULL &&
    (g_get_monotonic_time() - pstlhBTRCoreAVM->i64LastSelectConfigTime > BTR_SELECT_CONF_TIMEOUT))
    {
        BTRCORELOG_ERROR("Due to an error, a previous device has failed to connect after calling select configuration - resetting so the new device can connect\n");
        pstlhBTRCoreAVM->eAVMediaTypeOut = eBTRCoreAVMTypeUnknown;
    }
    else if (pstlhBTRCoreAVM->eAVMediaTypeOut == eBTRCoreAVMTypeSBC)
    {
        BTRCORELOG_ERROR("There is already an active SBC output device, function will not return configuration parameters, there may alse be another connection in progress\n");
    }
    ...
    if (aenBTMediaType == enBTMediaTypeSBC) {
        ...
        MEMSET_S(&lstBTMediaSbcConfig, sizeof(a2dp_sbc_t), 0, sizeof(a2dp_sbc_t));
        ...
        MEMSET_S(&lstBTMediaAacConfig, sizeof(a2dp_aac_t), 0, sizeof(a2dp_aac_t));
    ...
    if (*appBtMediaCapsOutput)
    {
        pstlhBTRCoreAVM->i64LastSelectConfigTime = g_get_monotonic_time();
    }
            ...
            if (pstlhBTRCoreAVM->pcAVMediaTransportPathIn) {
                if(!strncmp(pstlhBTRCoreAVM->pcAVMediaTransportPathIn, apBtMediaTransportPath, strlen(pstlhBTRCoreAVM->pcAVMediaTransportPathIn))) {
                    ...
                    if (lpstBTAVMMediaConfig) {
                        ...
                        MEMCPY_S(lpstBTAVMMediaConfig, MAX(MAX(sizeof(a2dp_aac_t), sizeof(a2dp_mpeg_t)), sizeof(a2dp_sbc_t)), &lstBtA2dpSbcCaps, sizeof(a2dp_sbc_t));
                    }
                }
            }
    ...
    if (pstlhBTRCoreAVM) {
        if (pstlhBTRCoreAVM->pcAVMediaPlayerPath) {
            ...
            if (!strncmp(pstlhBTRCoreAVM->pcAVMediaPlayerPath, apcBTMediaPlayerPath,
                ...
                MEMSET_S(&pstlhBTRCoreAVM->pstAVMediaPlayer, sizeof(pstlhBTRCoreAVM->pstAVMediaPlayer), 0, sizeof(pstlhBTRCoreAVM->pstAVMediaPlayer));
...
btrCore_AVMedia_MediaStatusUpdateCb (
    ...
    BTRCORELOG_TRACE ("AV Media Status Cb\n");
    ...
    case enBTMediaTransport:
        ...
        case enBTMedTPropState:
            ...
            BTRCORELOG_INFO ("AV Media Transport State : %d\n", pstlhBTRCoreAVM->eAVMTState);
            break;
        ...
        case enBTMedTPropVol:
            ...
            BTRCORELOG_INFO ("AV Media Transport Volume : %d%%\n", pstlhBTRCoreAVM->ui8AVMediaTransportVolume);
            postEvent = TRUE;
            break;
        case enBTMedTPropDelay:
            mediaStatus.eAVMediaState                   = eBTRCoreAVMediaPlyrDelay;
            mediaStatus.bIsAVMediaCtrlAvail             = pstlhBTRCoreAVM->bAVMediaPlayerConnected;
            mediaStatus.m_mediaPlayerTransportDelay     = apstBtMediaStUpdate->m_mediaTransportDelay;
            pstlhBTRCoreAVM->ui16AVMediaTransportDelay  = apstBtMediaStUpdate->m_mediaTransportDelay;
            pstlhBTRCoreAVM->bAVMediaTrDelayAvrcp       = TRUE;
            BTRCORELOG_INFO("AV Media Transport Delay : %d\n", pstlhBTRCoreAVM->ui16AVMediaTransportDelay);
            postEvent = TRUE;
            break;
        default:
            break;
                ...
                if (apstBtMediaStUpdate->m_mediaPlayerConnected) {
                    ...
                    */
                    if (!(pstlhBTRCoreAVM->bAVMediaPlayerConnected || pstlhBTRCoreAVM->pMediaPollingThread)) {
                        ...
                        MEMSET_S(pstAVMediaStUserData, sizeof(stBTRCoreAVMediaStatusUserData), 0, sizeof(stBTRCoreAVMediaStatusUserData));
                        MEMSET_S(&pstlhBTRCoreAVM->pstAVMediaPlayer, sizeof(stBTRCoreAVMediaPlayer), 0, sizeof(stBTRCoreAVMediaPlayer));
                        strncpy (pstlhBTRCoreAVM->pstAVMediaPlayer.m_mediaPlayerName, "UnknownPlayer", BTRCORE_MAX_STR_LEN -1);
    ...
    case enBTMediaPlayer:
        {
            ...
            case enBTMedPlayerPropName:
                ...
                safec_rc = strcpy_s(lstAVMediaPlayer->m_mediaPlayerName, BTRCORE_MAX_STR_LEN - 1, apstBtMediaStUpdate->m_mediaPlayerName); // CID 341391: String not null terminated (STRING_NULL)
                ERR_CHK(safec_rc);
                safec_rc = strcpy_s(mediaStatus.m_mediaPlayerName, BTRCORE_MAX_STR_LEN - 1, lstAVMediaPlayer->m_mediaPlayerName);
                ERR_CHK(safec_rc);
            ...
            case enBTMedPlayerPropType:
                ...
                BTRCORELOG_INFO ("AV Media Player Type : %d\n", lstAVMediaPlayer->eAVMediaPlayerType);
                break;
            ...
            case enBTMedPlayerPropSubtype:
                ...
                BTRCORELOG_INFO ("AV Media Player Subtype : %d\n", lstAVMediaPlayer->eAVMediaPlayerSubtype);
                break;
            ...
            case enBTMedPlayerPropTrack:
                ...
                */
                if (apstBtMediaStUpdate->m_mediaTrackInfo.pcTitle[0]) {
                    MEMSET_S(&lstAVMediaPlayer->m_mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo), 0, sizeof(stBTRCoreAVMediaTrackInfo));
                    MEMCPY_S(&lstAVMediaPlayer->m_mediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo), &apstBtMediaStUpdate->m_mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo));
                } else {
                ...
                MEMCPY_S(&mediaStatus.m_mediaTrackInfo,sizeof(stBTRCoreAVMediaTrackInfo), &lstAVMediaPlayer->m_mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo));
            ...
            case enBTMedPlayerPropPlaylist:
                ...
                if (pstlhBTRCoreAVM->pstAVMediaPlayList) {
                    ...
                    BTRCORELOG_INFO ("AV Media Player PlayList %p:%s [ID: %llu]\n", pstlhBTRCoreAVM->pstAVMediaPlayList
    ...
    if (ucItemScope) {
        ...
        if (apstBtMediaBsrUpdate->eMediaItemType == enBTMediaItemTypFolder ||
            ...
            if (!(i32BtRet = btrCore_AVMedia_AllocateBrowserMemory (pstlhBTRCoreAVM, ptr, &lpstBroswer))) {
                ...
                safec_rc = strcpy_s(lpstBroswer->pcAVMediaItemPath, BTRCORE_MAX_STR_LEN -1, apstBtMediaBsrUpdate->pcMediaItemPath);
                ERR_CHK(safec_rc);
                safec_rc = strcpy_s(lpstBroswer->pcAVMediaItemName, BTRCORE_MAX_STR_LEN -1, apstBtMediaBsrUpdate->pcMediaItemName); // CID 341569: String not null terminated (STRING_NULL)
                ERR_CHK(safec_rc);

                if ((lpstBroswer->bIsMediaItemPlayable = (apstBtMediaBsrUpdate->eMediaItemType == enBTMediaItemTypAudio))) {
                    MEMCPY_S(&lpstBroswer->mediaTrackInfo,sizeof(lpstBroswer->mediaTrackInfo), &apstBtMediaBsrUpdate->mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo));
                ...
                if ((mediaElement->bIsPlayable = lpstBroswer->bIsMediaItemPlayable)) {
                     MEMCPY_S(&mediaElement->m_mediaTrackInfo, sizeof(mediaElement->m_mediaTrackInfo),&lpstBroswer->mediaTrackInfo, sizeof(stBTRCoreAVMediaTrackInfo));
                }
                safec_rc = strcpy_s(mediaElement->m_mediaElementName, BTRCORE_MAX_STR_LEN -1, lpstBroswer->pcAVMediaItemName);
                ERR_CHK(safec_rc);
                postEvent = TRUE;
